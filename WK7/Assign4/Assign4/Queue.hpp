...
protected:
    ...
    struct Node
    {
        ...
        Node (Creature* v, Node* p = NULL, Node* n = NULL)
        {
...
public:
    Queue();    // Constructor
    ~Queue();   // Destructor
    void add(Creature*);  // Function to add a Node struct to the end of the Queue
    Creature* remove();   // Function to remove a Node struct from the begining of the Queue
    void display() const;   // Function used to display the Queue Nodes for testing
    bool Test();    // Function to check if Queue is empty
};
#endif /* Queue_hpp */
